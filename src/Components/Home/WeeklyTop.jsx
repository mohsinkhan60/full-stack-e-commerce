    ...
    <div className="container mx-auto max-w-full my-16 px-2 lg:px-4 xl:px-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stores.map((store, index) => (
          <div key={index} className={`${store.color} rounded-3xl p-6 flex flex-col items-center`}>
            ...
            <div className="flex flex-wrap justify-center p-2 mb-4 gap-2">
              {products.map((product, idx) => (
                <img key={idx} src={product} className="w-14 h-14 rounded-full bg-white object-cover" />
              ))}
            </div>
          </div>
        ))}
